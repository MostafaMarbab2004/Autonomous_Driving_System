...
def parse_keyboard(keys, current_steer):
    """ Converts W/A/S/D keypresses into continuous steer and accel actions """
    accel = 0.0
    ...
    # Steer: A (-1), D (+1)
    # We add a little smoothing so the steering wheel doesn't snap instantly
    steer_increment = 0.15
    if keys[pygame.K_a] or keys[pygame.K_LEFT]:
        current_steer -= steer_increment
    elif keys[pygame.K_d] or keys[pygame.K_RIGHT]:
        current_steer += steer_increment
    else:
        # Return to center automatically when letting go of keys
        if current_steer > 0:
            current_steer -= steer_increment
            current_steer = max(0.0, current_steer)
        elif current_steer < 0:
            current_steer += steer_increment
            current_steer = min(0.0, current_steer)
            
    # Clip steering between -1.0 and 1.0
    current_steer = np.clip(current_steer, -1.0, 1.0)
    
    # Return action [steer, accel]
    return np.array([current_steer, accel], dtype=np.float32), current_steer
...
def main():
    ...
    print(" - Use W/A/S/D or Arrow Keys to steer, throttle, and brake.")
    print(" - Follow the road and stay in the lane!")
    print(" - Press 'R' to manually restart the episode (saves data).")
    ...
    while running:
        ...
        action, current_steer = parse_keyboard(keys, current_steer)
